...
function Calendarize() {
	...
	return {
		...
		buildMonth: function(monthNum, year, opts) {
            ...
            $titleNode.classList.add('rounded-3');
			...
			if (opts.showMonth) {
				$titleNode.innerText = i18n(monthNames[monthNum]) + (opts.showYear ? " " + year : '');
			...
			if (opts.showDaysOfWeek) {
				dayNames.forEach(function(a, b) {
					...
					$dayNode.classList.add('font-bold');
					$dayNode.innerText = i18n(dayNames[b]).charAt(0);
